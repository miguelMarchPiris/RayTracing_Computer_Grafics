...
// TODO revisar metodo despues de hacer las pruebas pertinentes
void SceneFactoryVirtual::OneSphere(Scene *s) {
    Sphere * sphere0 = new Sphere(vec3(0, 0, -1), 0.5, 1.0);
    sphere0->setMaterial(new Lambertian(vec3(0.5, 0.5, 0.5))); //material de component difosa
    Sphere * sphere1 = new Sphere(vec3(0, -100.5, -1), 100, 1.0);
    sphere1->setMaterial(new Lambertian(vec3(0.8, 0.8, 0)));

    sphere0->getMaterial()->Kspecular = vec3(1, 1, 1);
    sphere1->getMaterial()->Kspecular = vec3(1, 1, 1);
